...
const LiveRideTracking = ({ ride, userType }) => {
    ...
    const watchId = navigator.geolocation.watchPosition(
      (position) => {
        ...
        if (previousPosition.current) {
          const calculatedHeading = calculateHeading(previousPosition.current, newPos);
  ...
  useEffect(() => {
    if (userType !== "captain" || !ride?._id || !ride?.captain || !currentPosition) return;

    const captainId = typeof ride.captain === 'string' ? ride.captain : ride.captain._id;
  ...
  const mapCenter = userType === "captain" 
    ? currentPosition 
  ...
  if (loadError) {
    return (
      <div className="w-full h-full flex items-center justify-center bg-red-50">
        <p className="text-red-600">Error loading maps. Please refresh the page.</p>
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      {currentPosition && (
        <Marker
          position={userType === "captain" ? currentPosition : (captainPosition || currentPosition)}
          icon={{
