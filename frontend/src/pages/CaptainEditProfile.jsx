...
const CaptainEditProfile = () => {
  ...
  useEffect(() => {
    if (captainData && Object.keys(captainData).length > 0) {
      setFirstname(captainData?.fullname?.firstname || "");
      setLastname(captainData?.fullname?.lastname || "");
      setImage(captainData?.profilepic || "");
      setVehicleColor(captainData?.vehicle?.color || "");
      setVehicleType(captainData?.vehicle?.vehicletype || "");
      setVehiclePlate(captainData?.vehicle?.plate || "");
      setVehicleCapacity(captainData?.vehicle?.capacity || "");
    }
  }, [captainData]);
  ...
  const handleSendOtp = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setOtpLoading(false);
        toast.success(
          `OTP sent to ${captainData.email}. Please check your inbox.`,
        );
      }
    } catch (error) {
      setOtpLoading(false);
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      const response = await axios.post(
        ...
        { email: captainData.email, otp: otp },
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
  ...
  const handleUpdateProfile = async (e) => {
    ...
    try {
      ...
      formData.append("vehiclePlate", vehiclePlate.toUpperCase());
      formData.append("vehicleCapacity", vehicleCapacity);

      const response = await axios.put(
        `${import.meta.env.VITE_BASE_URL}/captains/edit-profile`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      const response = await axios.delete(
        `${import.meta.env.VITE_BASE_URL}/captains/delete-captain`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
        ...
        <div className="max-w-md mx-auto lg:max-w-7xl">
          ...
          <div className="lg:grid lg:grid-cols-3 lg:gap-8">
                      ...
                      <div className="space-y-3">
                        <div className="flex items-center gap-3 text-sm">
                          ...
                          <span className="text-gray-600 font-light truncate">
                            {captainData?.email || "Not set"}
                          </span>
                        </div>
                        <div className="flex items-center gap-3 text-sm">
                          <i className="ri-phone-line text-gray-400"></i>
                          <span className="text-gray-600 font-light">
                            {captainData?.mobile || "Not set"}
                          </span>
                        </div>
                        {vehiclePlate && (
                          <div className="flex items-center gap-3 text-sm">
            ...
            <div className="lg:col-span-2">
              <div className="bg-white rounded-2xl shadow-lg md:border border-gray-200 p-6 sm:p-8 lg:p-8">
                      ...
                      <div className="space-y-5 lg:space-y-0 lg:grid lg:grid-cols-2 lg:gap-5">
                        ...
                        <div>
                          ...
                          <select
                            ...
                            className="w-full bg-white border border-gray-200 rounded-lg px-4 py-3 text-base text-gray-900 focus:outline-none focus:ring-2 focus:ring-black focus:border-transparent transition-all"
                            required
                          >
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 sm:p-8">
            <div className="text-center mb-6">
              ...
              <p className="text-gray-500 text-sm">
                This action cannot be undone. All your captain data will be
                permanently deleted.
              </p>
            </div>
